...
def MFCC_1vowel_nspeaker(vowel_label, K=3):
    ...
    K_vectors = kmeans.cluster_centers_   # tam cum
    ...
    for i in range(K):
        ...
        cluster_points = np.array(all_vectors)[labels == i]
        # Trực quan hóa các điểm đó
        plt.scatter(cluster_points[:, 0], cluster_points[:, 1], label=f'group {i}')
    # Vẽ các tâm cụm
    plt.scatter(K_vectors[:, 0], K_vectors[:, 1], s=300, c='red', label='Centroids', marker='*')
    # Thêm một số tính năng cho đồ thị
    plt.title(f'Points in each group {vowel_label}')
...
def matching(vector_x, model_vectors):
    ...
    * Đầu ra: nhãn
    ...
    distances = [np.linalg.norm(vector_x - vector_clus) for vowel_vectors in model_vectors for vector_clus in
                 vowel_vectors]
